...
// 100 us
#define MONITOR_SPIN_BACKOFF_NS             100000L
...
// --- Sniffer integration
// Signaled by monitorRecvBuffer right after it copies new bytes into host
...
void *monitorRecvBuffer(void *arg){
    ...
    while(thread_control){
        ...
        if(prudata[SHRAM_OFFSET_485_MODE] == 'M'){
            ...
            // (see read_lenfifo_count()'s comment) will probably never
            // reproduce the exact same wrong value twice in a row, so
