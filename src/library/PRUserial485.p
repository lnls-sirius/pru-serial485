...
// Sniffer (Slave mode): on-chip length-FIFO counter and ring, both in
// shared RAM (reusing the otherwise-idle "Sending Data" region, since
// the sniffer never transmits). Keep in sync with shram_mapping.h.
#define OFFSET_SHRAM_LENFIFO_COUNT          100
#define OFFSET_SHRAM_LENFIFO_RING           104
#define LENFIFO_DEPTH_MASK                  0x7FF                   // LENFIFO_DEPTH(2048) - 1
...
// Configura I para contar a quantidade de bytes recebido
    ZERO        &I, 4
...
// same as before; nothing in this block touches DDR anymore.
    LBCO        DDR_POINTER, SHRAM_BASE, OFFSET_SHRAM_LENFIFO_COUNT, 4  // monotonic count so far
    MOV         J, LENFIFO_DEPTH_MASK
    AND         K, DDR_POINTER, J
    LSL         K, K, 1                                          // 2 bytes/entry
    ADD         K, K, OFFSET_SHRAM_LENFIFO_RING
    SBCO        I, SHRAM_BASE, K, 2                               // write this message's length
    ADD         DDR_POINTER, DDR_POINTER, 1
    SBCO        DDR_POINTER, SHRAM_BASE, OFFSET_SHRAM_LENFIFO_COUNT, 4  // publish new count
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
// ************* VERIFICA MENSAGEM DE SINCRONISMO
    QBNE        DATA_READY_SLAVE, I, 0x06                       // Se tamanho == 6, pode ser um broadcast sincronismo
