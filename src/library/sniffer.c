...
#define SNIFFER_WAIT_TIMEOUT_NS    100000000L   // 100 ms
...
static char   sniffer_log_current_name[256];      // basename only, e.g. "sniff_...log"
static size_t sniffer_log_bytes_written = 0;      // bytes in the *current* file (for rotation)
static size_t sniffer_rotate_bytes = 0;
static size_t sniffer_max_total_bytes = 0;        // 0 = unlimited; total across all rotated files
    ...
    for(;;){
        ...
        while((entry = readdir(dir)) != NULL){
            ...
            if(strcmp(entry->d_name, sniffer_log_current_name) == 0){
                continue;   // never a deletion candidate while active
            }
        ...
        if(!have_oldest){
            return;   // nothing left we're allowed to delete
        }
...
static void sniffer_log_write_raw(const uint8_t *buf, size_t len){
    ...
    if(sniffer_rotate_bytes > 0 && sniffer_log_bytes_written >= sniffer_rotate_bytes){
        fsync(sniffer_log_fd);   // always flush fully before rotating away from this fd
...
static void sniffer_log_write_event(uint8_t magic, uint32_t count){
    ...
    header[1] = SNIFFER_LOG_VERSION;
    write_u64_le(&header[2], ts_ns);
    write_u32_le(&header[10], sniffer_seq);      // ties event to the next real message's seq
...
static void *sniffer_capture_thread(void *arg){
    ...
    while(sniffer_running && !sniffer_write_failed){
        ...
        if(new_count == 0){
            continue;   // timed out with nothing new yet, or spurious wakeup
        }
        ...
        new_count = k;   // however many we decided to actually process this round
        ...
        for(k = 0; k < new_count; k++){
            if(offset + entry_lengths[k] > got_bytes){
                break;   // ran out of real bytes for this message
            }
...
void PRUserial485_sniffer_stop(void){
    ...
    if(sniffer_log_fd >= 0){
        fsync(sniffer_log_fd);   // flush whatever's pending from a partial batch
        close(sniffer_log_fd);
        sniffer_log_fd = -1;
    }
}
